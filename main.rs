...
fn main() {
	let args: Vec<String> = env::args().collect(); // аргументы при запуске программы
    if (args.len() == 3) && (args[1] == "ip:port") {  //режим клиента
        let ip = &args[2];
                    ...
                    Ok(_) => {
                        ...
                        if received_key == next_key {
                            loop {
                                ...
                                let mut message: String = "test123_-+qweQWE!@#".to_string(); //сообщение, отправляемое на сервер
        ...
        if (args.len() == 5) && (args[1] == "port")  && (args[3] == "-n") { // режим сервера
            let port = &args[2];
            ...
            for stream in listener.incoming() {
                match stream {
                    Ok(stream) => {
                        ...
                        pool.execute(move || {
                            let mut con_stream = stream;                       
                            let mut hash_message = [0 as u8; 15]; 
                            let mut message = [0 as u8; 60];
                            match con_stream.read(&mut hash_message) {
                                Ok(_) => {
                                    ...
                                    con_stream.write(&result).unwrap();
                                    loop { match con_stream.read(&mut message) {
                                        Ok(_) => {
                                            ...
                                            println!("Message from client {} Received: {}\nReceived key: {}\nGenerated key: {}\n", con_stream.peer_addr().unwrap(), &txt, &received_key, &new_key);
                                            result = (new_key + &txt.to_uppercase()).clone().into_bytes();
                                            con_stream.write(&result).unwrap();
                                        },
                                        Err(e) => {
                                            println!("Connection error with {}\nError: {}", con_stream.peer_addr().unwrap(), e);
                                            con_stream.shutdown(Shutdown::Both).unwrap();
                                            break
                                        }
                                    } }
                                },
